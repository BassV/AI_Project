...
public class TableSetup : MonoBehaviour
{
    ...
    private void CalculateTextureColors()
    {
        for (int y = 0; y < worldSizeY; y++)
        {
            for (int x = 0; x < worldSizeX; x++)
            {
                ...
                {
                    colorcontainer[y * worldSizeX + x] = Color.red;
                }
                else
                {
                    colorcontainer[y * worldSizeX + x] = Color.green;
                }
    ...
    private void ApplyTexture()
    {
        ...
        Renderer rend = GameObject.Find("CollisionIndicator").GetComponent<Renderer>();
